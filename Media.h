...
protected:
  ...
  virtual void set_status(uint8_t new_status){
    status_mtx.lock();
    status = new_status;
    status_mtx.unlock();
  }
...
public:
  ...
  virtual const uint8_t* get_font() const {
    if (__DEBUG) Serial.println("Reached Virtual Method!!!!!!");
    assert(false);
  }
...
public: 
  Text(String input, const size_t duration, const uint8_t* ft, const uint16_t cx, const uint16_t cy)
  ...
  Text(char* input, const size_t duration, const uint8_t* ft, const uint16_t cx, const uint16_t cy)
    : MediaContainer(MEDIA_TEXT, duration)
